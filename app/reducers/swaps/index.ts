...
function processToken(token: Record<string, unknown>) {
  ...
  return {
    ...
    address: String(tokenData.address).toLowerCase(),
  };
}
...
function addMetadata(
  ...
  return tokens.map((token: Record<string, unknown>) => {
    const tokenMetadata =
      tokenList?.[safeToChecksumAddress(String(token.address)) || ''];
    if (
      tokenMetadata &&
      typeof tokenMetadata === 'object' &&
      'name' in tokenMetadata
    ) {
      return { ...token, name: (tokenMetadata as { name: string }).name };
    }
...
export const swapsSmartTxFlagEnabled = createSelector(
  ...
  (swapsState) => {
    ...
    const smartTransactions = globalFlags?.smartTransactions as
      | { mobileActive?: boolean }
      | undefined;
    const isEnabled = Boolean(smartTransactions?.mobileActive);
    return isEnabled;
  },
);
  ...
  switch (action.type) {
    case SWAPS_SET_LIVENESS: {
      ...
      const chainNoFlags: SwapsChainState = {
        isLive: false,
        featureFlags: {},
        ...(data && typeof data === 'object' && 'isLive' in data ? data : {}),
      };
      ...
      Object.keys(featureFlags).forEach((chainName) => {
        ...
        if (
          ...
          const existingState = state[chainIdForName] as
            | SwapsChainState
            | undefined;
          (newState as Record<string, unknown>)[chainIdForName] = {
            isLive: chainLiveness,
            featureFlags: chainFeatureFlags as Record<string, unknown>,
            ...(existingState &&
            typeof existingState === 'object' &&
            'isLive' in existingState
              ? existingState
              : {}),
          };

          if (chainIdForName === chainId && rawChainId !== chainId) {
            (newState as Record<string, unknown>)[rawChainId] = (
