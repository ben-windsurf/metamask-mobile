...
export interface SwapsState {
  ...
  [chainId: string]: unknown | SwapsChainState;
}

export interface SetSwapsLivenessPayload {
  chainId: string;
  featureFlags: FeatureFlags | null;
}

export interface SetSwapsHasOnboardedPayload {
  hasOnboarded: boolean;
}
 ...
 */
function processToken(token: Record<string, unknown>) {
  if (!token) return null;
  const { hasBalanceError, image, ...tokenData } = token;
  return {
    occurrences: 0,
    ...tokenData,
    decimals: Number((tokenData as Record<string, unknown>).decimals),
    address: String(
      (tokenData as Record<string, unknown>).address,
    ).toLowerCase(),
  };
}
 ...
 */
function combineTokens(tokenSources: Record<string, unknown>[][]) {
...
function addMetadata(
  ...
  tokenList: Record<string, Record<string, unknown>>,
) {
  ...
  return tokens.map((token: Record<string, unknown>) => {
    ...
    if (tokenAddress && tokenList) {
      const checksumAddress = safeToChecksumAddress(String(tokenAddress));
...
export const swapsSmartTxFlagEnabled = createSelector(
  ...
  (swapsState) => {
    ...
    const isEnabled = Boolean(
      (globalFlags?.smartTransactions as Record<string, unknown>)?.mobileActive,
    );
    return isEnabled;
  },
);
  ...
  (swapsTokens, allTokens, currentUserAddress) => {
    ...
    if (allTokens && currentUserAddress) {
      for (const chainId in allTokens) {
        ...
        if (
          !chainTokens ||
          !(chainTokens as Record<string, unknown>)[currentUserAddress]
        )
          continue;
...
export const swapsTokensObjectSelector = createSelector(
  ...
  (tokens) => {
    ...
    for (const token of tokens) {
      result[String((token as Record<string, unknown>).address)] = undefined;
    }
    return result;
  },
);
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensAddressesWithBalance = Object.entries(balances)
      .filter(([, balance]) => balance && String(balance) !== '0')
    ...
    const result = topAssetsData
      .map(({ address }: Record<string, unknown>) =>
        tokens?.find((token: Record<string, unknown>) =>
          areAddressesEqual(String(token.address), String(address)),
        ),
      )
      .filter(Boolean);
    return addMetadata(chainId, result, tokenList);
...
const slice = createSlice({
  ...
  reducers: {
    setSwapsLiveness: (
      state,
      action: PayloadAction<SetSwapsLivenessPayload>,
    ) => {
      ...
      if (!featureFlags) {
        state[chainId] = chainNoFlags;
        state[rawChainId] = chainNoFlags;
      ...
      Object.keys(featureFlags).forEach((chainName) => {
        ...
        if (
          ...
          state[chainIdForName] = {
            ...(state[chainIdForName] as SwapsChainState),
            featureFlags: chainFeatureFlags,
            isLive: chainLiveness,
          };

          if (chainIdForName === chainId && rawChainId !== chainId) {
            state[rawChainId] = state[chainIdForName];
          }
        }
      });
    },
    setSwapsHasOnboarded: (state, action: PayloadAction<boolean>) => {
...
export const setSwapsLiveness = (
  chainId: string,
  featureFlags: FeatureFlags | null,
) => actions.setSwapsLiveness({ chainId, featureFlags });

export const setSwapsHasOnboarded = (hasOnboarded: boolean) =>
  actions.setSwapsHasOnboarded(hasOnboarded);
